$(document).ready(function() {
    ...
    var sale_out_imei_number = $(".sale_out_imei_number").val();
    // alert(sale_out_imei_number);
    var stock_report_cols = [];

    if ($('th.action_current_stock_enter').length) {
        stock_report_cols.push({ data: 'action', name: 'action', searchable: false });
    }
    stock_report_cols.push({ data: 'sub_sku', name: 'sub_sku' });
    stock_report_cols.push({ data: 'name', name: 'p.name' });
    // new sale_imei_number
    if (sale_out_imei_number == 29) {
        stock_report_cols.push({ data: 'sale_imei_number', name: 'sale_imei_number' });
    }
    stock_report_cols.push({ data: 'variation', name: 'variation', searchable: false });
    stock_report_cols.push({ data: 'category_name', name: 'category_name', searchable: false });
    stock_report_cols.push({ data: 'location_name', name: 'location_name', searchable: false });
    stock_report_cols.push({ data: 'unit_price', name: 'unit_price', searchable: false });
    stock_report_cols.push({ data: 'stock', name: 'stock', searchable: false, searchable: false });
    ...
    // By Zaheer
    if ($('th.current_stock_enter').length) {

        stock_report_cols.push({
            data: null, name: null, searchable: false, orderable: false,
            render: function (data, type, row) {

                return '<form id="myForm" action="/save-data-stock" method="POST">' +
                    '<meta name="csrf-token" content="{{ csrf_token() }}">' +
                    '<input type="hidden" class="control-form" name="product_id" value="' + data.product_id + '">' + // Removed extra space
                    '<input type="hidden" class="control-form" name="variation_id" value="' + data.variation_id + '">' +
                    '<input type="number" class="dollar-input control-form" name="enter_current_stock">' +
                    '<button type="submit" id="submitButton" class="btn btn-success custom-button" style="margin-top: 14px;">Submit</button>' +
                    '</form>';
            }
        });
    }
    ...
    $(
        '#psr_customer_group_id, #psr_filter_category_id, #psr_filter_brand_id, #product_sell_report_form #variation_id, #product_sell_report_form #location_id, #product_sell_report_form #customer_id , #user_id'
    ).change(function() {
        product_sell_report.ajax.reload();
        product_sell_grouped_report.ajax.reload();
        product_sell_report_with_purchase_table.ajax.reload();

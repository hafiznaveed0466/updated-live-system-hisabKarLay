$(document).ready(function () {
    ...
    $('.asad').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#customer").html('Customer:' + ' ' + data[0].name);
                ...
                $.each(data, function (key, value) {
                    res +=
                        '<tr>' +
                        '<td>' + value.pro_name + '(' + value.var_name + ')' + '</td>' +
    ...
    $('.btnprn1').on('click', function () {

        var ButtonControl = document.getElementById("btnprn1");
            ...
            .autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so) {
                        ...
                        var selectElement = document.getElementById("pricedata");

                        // Get the selected value
                        if (selectElement !== undefined && selectElement !== null && selectElement !== "") {
                            var selectedValue = selectElement.value;
                        } else {
                            var selectedValue = "pkr";
                        }
                        ...
                        var imei_number = ui.item.purchase_line_id && searched_term == ui.item.imei_number ? ui.item.imei_number : null;

                        pos_product_row(ui.item.variation_id, purchase_line_id, selectedValue, imei_number);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function () {
        ...
        // alert("change in unit price update price including tax and line total");
...
// alert(" i am in tax rate");
        ...
        var discounted_unit_price = __get_principle(unit_price, tax_rate);
        var pos_unit_price_inc_tax = get_unit_price_from_discounted_unit_price(tr, discounted_unit_price);


        __write_number(tr.find('input.pos_unit_price_inc_tax'), pos_unit_price_inc_tax);

        pos_each_row(tr);
        var unit_price_incl = __read_number(tr.find('input.pos_unit_price_inc_tax'));
        var line_total = quantity * unit_price_incl;
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function () {
        ...
        // alert("change in unit price including tax, update unit price");
    ...
    $('table#pos_table tbody').on('click change', 'input.is_flex_width, input.is_flex_height ,input.qty_flex', function () {

        var width = $(this).closest('tr').find('input.is_flex_width').val();
        var height = $(this).closest('tr').find('input.is_flex_height').val();
        var qty_flex = $(this).closest('tr').find('input.qty_flex').val();


        if (width > 0 && height > 0 && qty_flex > 0) {
            var quantity = height * width
            var qty_flex = quantity * qty_flex
            var quantity = qty_flex + ".00"
            $(this).closest('tr').find('input.pos_quantity').val(quantity);
            $($(this).closest('tr').find('input.pos_quantity')).trigger('change')
        }
    })
    ...
    $('table#pos_table tbody').on(
        ...
        function () {
            var tr = $(this).parents('tr');
            // alert("Change in row discount type or discount amount");
            ...
            if (row_discount_type === "fixed") {
                ...
                var row_discount_amount1 = __read_number(tr.find('input.row_discount_amount'));
                var pos_unit = __read_number(tr.find('input.pos_unit_price_inc_tax'));

                var unit_price_inc_tax = pos_unit - row_discount_amount1;
                var line_total = quantity * unit_price_inc_tax;
            } else {
            ...
            // alert("unit_price_inc_tax "+unit_price_inc_tax);
            // alert("line_total "+line_total);
    ...
    $('button.pos-express-finalize').click(function () {
        ...
        else if (pay_method == 'express_checkout') {
            $('div#confirmSuspendModal').modal('show');
        } else {
            pos_form_obj.submit();
        }
    ...
    $('button#pos_suspend_bill').click(function () {
        $('input.is_suspend').val(1);
        $('div#confirmSuspendModal').modal('hide');
        pos_form_obj.submit();
        $('.print_window_bill').show();
        $('input.is_suspend').val(0);
        $.ajax({
            method: 'GET',
            url: '/kot_bill_ids',
            ...
            success: function (data) {
                ...
                $('.billkot').html(res);
                $('.print_window_bill').printThis({
                });
                setTimeout(function () {
                    $('.print_window_bill').hide();
                }, 2000);
            },
        });
    });

    // $('button#pos_kot_bill').click(function () {
    //     $('input#is_suspend').val(1);
    //     $('div#confirmSuspendModal').modal('hide');
    //     pos_form_obj.submit();
    //     $('input#is_suspend').val(0);
    //     $('.print_window_bill_kot').show();
    //     $('.KOT_IDD').show();
    //     $.ajax({
    //         method: 'GET',
    //         url: '/kot_bill_pos',
    ...
    //             $(".business_kot").html(data[0].bname);


    //             $("#customer_kot").html('Customer:' + ' ' + data[0].cname);
    //             $("#status_kot").html('Invoice_No:' + ' ' + data[0].invoice_no);
    //             $("#mobile_kot").html('Mobile:' + ' ' + data[0].mobile);


    //             $("#date_kot").html('Date:' + ' ' + f);
    //             $("#Subtotal_kot").html('Rs:' + ' ' + parseFloat(data[0].final_total));
    //             $("#address_kot").html(data[0].landmark + '  ' + data[0].city + '    ' + data[0]
    //                 .zip_code + '   ' + data[0].state + '  ' + data[0].country);
    ...
    //             $('#billkot_kot').html(res);
    //             $('.print_window_bill_kot').printThis({

    //             });
    //             setTimeout(function () {
    //                 $('.print_window_bill_kot').hide();
    //             }, 2000);
    //         }
    //     });
...
function pos_product_row(variation_id = null, purchase_line_id = null, selectedValue = null, imei_number, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            method: 'GET',
            url: '/sells/pos/get_product_row/' + variation_id + '/' + location_id + '/' + selectedValue,
            async: false,
            data: {
...
function pos_each_row(row_obj) {
    ...
    var tax_rate = row_obj
        ...
        var unit_price_inc_tax =
            ...
            // alert("in pos each "+unit_price_inc_tax)
    ...
    $('#payment_rows_div')
        ...
        .each(function () {
            if (parseFloat($(this).val())) {
                ...
                $(document).ready(function () {
                    var data = document.getElementById('net_cash').value;
                    if (data) {
...
function reset_pos_form() {
    ...
    if ($('#due_date').length > 0) {
        $('#due_date').data("DateTimePicker").date(moment());
    }
    if ($('.paid_on').length > 0) {
...
$(document).ready(function () {
    ...
    if (window.location.pathname.includes("/pos/219/edit")) {
...
//  $('.category_box').on('click', function(e) {
//     $('input#suggestion_page').val(1);
//     var location_id = $('input#location_id').val();
//     if (location_id != '' || location_id != undefined) {
//         get_product_suggestion_list(
...
$(document).ready(function () {
    ...
    get_location_based_category_list(location_id);

});

function get_location_based_category_list(location_id) {
...
$('#select_location_id').on('change', function (e) {
    $('#product_category').val(1);
    var location_id = $('input#location_id').val();
    if (location_id != '' || location_id != undefined) {
        get_location_based_category_list(

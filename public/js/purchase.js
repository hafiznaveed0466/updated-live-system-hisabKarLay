$(document).ready(function() {
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        scrollCollapse: true,
        buttons: [
            { extend: 'csv', text: '<i class="fa fa-file-csv" aria-hidden="true"></i> Export to CSV' },
            {
                extend: 'excel', text: '<i class="fa fa-file-excel" aria-hidden="true"></i> Export to EXCEL', exportOptions: {
                    columns: ':visible:not(:eq(0))',
                    rows: ':not(:eq(1))',
                }
            },
            { extend: 'print', text: '<i class="fa fa-print" aria-hidden="true"></i> Print' },
            { extend: 'colvis', text: '<i class="fa fa-columns" aria-hidden="true"></i> Column visibility' },
            {
                extend: 'pdf',
                text: '<i class="fa fa-file-pdf" aria-hidden="true"></i> Export to PDF',
                exportOptions: {
                    columns: ':visible:not(:eq(0))', // Exclude the first and second visible columns
                    rows: ':not(:eq(1))', // Exclude the third row
                }

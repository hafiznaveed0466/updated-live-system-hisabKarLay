...
class TransactionUtil extends Util
{
     ...
     */
        ...
        if (!empty($input['sale_note_whole_sale'])) {
            $additional_notes = $input['sale_note_whole_sale'];
        } else {
            $additional_notes = !empty($input['sale_note']) ? $input['sale_note'] : null;
        }
        if (!empty($input['staff_note_whole_sale'])) {
            $staff_note = $input['staff_note_whole_sale'];
        } else {
        ...
        if(!empty($input['due_dates'])){
            ...
            $dueDates= strtotime( $input['due_dates']); 
            
            $due_date =  date('d-m-Y h:i', $dueDates );
        ...
        // Auto D.o Number
        $DO_nUMBER = str_pad(mt_rand(1, 9999), 4, '0', STR_PAD_LEFT);
        ...
        $transaction = Transaction::create([
            ...
            'currency_type' => isset($input['curencychange']) ? $input['curencychange'] : null,
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) 
        {
            ...
            //   //flex
            if (isset($product['width']) && isset($product['height'])) {
                $width  = $product['width'];
                $height = $product['height'];
            }
            if (isset($product['qty_flex'])) {
                $qty_flex  = $product['qty_flex'];
            }
            ...
            }elseif(!empty($product['product_type']) && $product['product_type'] == 'modifier'){
            ...
            if(isset($product['parent_sell_line_id']) && !empty($product['parent_sell_line_id'])) {
                $parent_sell_line_id = $product['parent_sell_line_id'];
            } else {
                $parent_sell_line_id = NULL; // Set it to NULL if it's empty or not set
            }
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'imei_number' => !empty($product['imei_number']) ? $product['imei_number'] : null,
                    ...
                    if (!empty($product['modifier'])) {
                        foreach ($product['modifier'] as $key => $value) {
                            ...
                            }else
                            {
                                ...
                                $modifier_price = $value['modifier_price'];
                                $modifier_quantity = isset($product['modifier_quantity'][$key]) ? $product['modifier_quantity'][$key] : 1;
                                $modifier_set_id = $value['modifier_set_id'];
                                
                                // Now you can use these variables to create the modifier array
                                // for each modifier and add it to $sell_line_modifiers
                                
                                $this_price = $uf_data ? $this->num_uf($modifier_price) : $modifier_price;
                                $sell_line_modifiers[] = [
                                    'product_id' => $modifier_set_id,
                                    'variation_id' => $modifier_id,
                                    'quantity' => $modifier_quantity,
                                    'unit_price_before_discount' => $this_price,
                                    'unit_price' => $this_price,
                                    'unit_price_inc_tax' => $this_price,
                ...
                $this->updateSalesOrderLine($line['so_line_id'], $line['quantity'], 0, $line['height'], $line['width'], $line['qty_flex']);
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        if ($status_before != 'draft') {
            ...
            $this->adjustQuantity($location_id, $product['product_id'], $product['variation_id'], $difference , $product['transaction_sell_lines_id']);
        }
        ...
        $sell_line->fill([
            ...
            'sub_unit_id' => !empty($product['sub_unit_id']) ? $product['sub_unit_id'] : null,
            'res_service_staff_id' => !empty($product['res_service_staff_id']) ? $product['res_service_staff_id'] : null,
            'height' => !empty($product['height']) ? $product['height'] : null,
            'width' => !empty($product['width']) ? $product['width'] : null,
            'qty_flex' => !empty($product['qty_flex']) ? $product['qty_flex'] : null,
        ]);
        $sell_line->save();
        ...
        $this->updateSalesOrderLine($sell_line->so_line_id, $sell_line->quantity, $old_qty, $sell_line->height, $sell_line->width, $sell_line->qty_flex);
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {
        ...
        if (!empty($transaction_line_ids)) {
            ...
            //Adjust quanity
// dd( $sell_lines , $transaction_line_ids);
            foreach ($sell_lines as $line) {
                if ($adjust_qty) {
                    
                    // $this->adjustQuantity($location_id, $line->product_id, $line->variation_id, $line->quantity , $line->transaction_sell_lines_id);
                    $this->adjustQuantityDelete($location_id, $line->product_id, $line->variation_id, $line->quantity);
                }
     ...
     * @return boolean
     */
    private function adjustQuantity($location_id, $product_id, $variation_id, $increment_qty , $transaction_sell_lines_id)
    {       
        if ($increment_qty != 0) {
            $enable_stock = Product::find($product_id)->enable_stock;
            
            if (($enable_stock == 1 ) && (!empty($transaction_sell_lines_id))) {
    ...
    // for delete sell
    private function adjustQuantityDelete($location_id, $product_id, $variation_id, $increment_qty)
    {       
        if ($increment_qty != 0) {
            $enable_stock = Product::find($product_id)->enable_stock;
    ...
    public function createOrUpdatePaymentLine($transaction, $payments, $uf_data = true)
    {
        // dd($$transaction);
        $user = Auth::user();
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type,$business_id)
    {
        ...
        $output = [
            ...
            //flex
            'is_flex_enable' => $business_details->enable_is_flex,
        ...
        $output['do_number'] = $transaction->do_number;
        $output['transaction_date'] = $transaction->transaction_date;
        $output['date_time_format'] = $business_details->date_format;
        if (!empty($business_details->enable_is_other_currency)) {
            if ($transaction->currency_type == 'usd') {
                $output['currency_symbol'] = "$";
            }elseif($transaction->currency_type == 'aed'){
                $output['currency_symbol'] = "$";
            }else{
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
           ...
           if ($business_id == 83) {
                $output['subtotal_exc_tax'] = ' PKR'. $subtotal_exc_tax;
        ...
        $padding_length = 10; // Total length including the currency symbol and the total
        $padding_character = ' ';

        $output['subtotal'] = str_pad($currency, $padding_length, $padding_character, STR_PAD_BOTH) . $total;
                } else {
        ...
        if (!empty($output['taxes'])) {
            ...
            foreach ($output['taxes'] as $key => $value) {
                ...
                if ($business_id == 89) {
                    ...
                    $roundedValue = bcadd($value, 0, 2); // Round the value to 2 decimal places

                    if (substr($roundedValue, -1) == '9') {
                        $roundedValue = bcadd($roundedValue, '0.01', 2);
                    }
                
                    $formattedValue = number_format($roundedValue, 2, '.', ''); // Format to 2 decimal places
                    $output['taxes'][$key] = $formattedValue;

                    // dd($output['taxes'][$key] , "if");

                }else {
                $output['taxes'][$key]= $value;
                // dd($output['taxes'][$key] , "else");
                }
            }
             if ($business_id == 89) {
              
            }else {
                $output['taxes'][trans("lang_v1.total_tax")] = $this->num_f( $show_currency, $business_details);
            }
        ...
        if ($transaction_type == 'sell_return') {
         ...
         if ($business_id == 83) {
            ...
            $output['total'] = str_pad($currency, $padding_length, $padding_character, STR_PAD_BOTH) . $total;
                        } else {
                
                 if($business_id == 89){
                $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details). "0" ;
            }else{
                $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);    
            }
                // $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
...
$padding_length = 12; // Total length including the currency symbol and the value
$padding_character = ' ';

$output['all_due'] = $currency . str_pad($all_due, $padding_length - strlen($currency), $padding_character, STR_PAD_LEFT);
                    }else{
                        $output['all_due'] = $this->num_f($all_due, $show_currency, $business_details);
                    }
                         $output['dues'] =$all_due-$transaction->final_total ;
                }
            }
        ...
        $output['footercompany_text'] = $invoice_layout->footercompany_text;
    ...
    public function getDiscountTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
     {
         $query = DB::table('transactions as t')
           ...
           ->select(
                 DB::raw('ROUND(
                     (SUM(
                         CASE WHEN t.discount_type = "fixed" THEN t.discount_amount
                              ELSE t.discount_amount * 100 / t.total_before_tax
                         END
                     ) +
                     SUM(
                         CASE WHEN tsl.line_discount_type = "fixed" THEN tsl.line_discount_amount
                              ELSE tsl.line_discount_amount * 100 / tsl.unit_price_before_discount
                         END
                     )), 2
                 ) AS total_discounts')
             );
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        $output['totals_due'] = 
        $purchase_details->total_paid - $purchase->paid_on;
    ...
    // public function getPaidDue($business_id, $start, $end, $location_id, $type)
    // {
    //     $query  = Transaction::where('transactions.business_id', $business_id)
    //         ->where('transactions.type', $type);

    //     // $paidDues = $query->with(['payment_lines' => function ($q) {
    //     //     $q->where('is_paid_due', 1);
    //     // }])->get();

    //     // $total = 0;
    //     // foreach ($paidDues as $paidDue) {
    //     //     if (isset($paidDue->payment_lines)) {
    //     //         foreach ($paidDue->payment_lines as $payment) {
    ...
    public function getpaid($business_id,$start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {   
       
        $query = DB::table('transactions')->where('transactions.business_id', $business_id)
    ...
    // Import getTotalPaidImport
     public function getTotalPaidImport($transaction_id)
    {
        ...
        {
          return  $this->markTransactionAsPaid($transaction_id);
        }
 ...
 public function markTransactionAsPaid($transaction_id)
{
    $transaction = Transaction::find($transaction_id);

    $transaction_final_total = $transaction->final_total;

   return  $this->createTransactionPayment($transaction, $transaction_final_total);
}

public function createTransactionPayment($transaction, $amount)
{
    // dd($transaction_id, $amount);
    // Create a new TransactionPayment record
    ...
    $transactionPayment->business_id = $transaction->business_id;
    ...
    // Import for sale
        public function calculatePaymentStatusupdateImport($transaction_id, $final_amount = null , $import_batch = null)
    {
    
        $total_paid = $this->getTotalPaidImport($transaction_id);
        ...
        if(!empty($total_paid['amount']) || !empty($import_batch)){
...
// Import updatePaymentStatusImport
    public function updatePaymentStatusImport($transaction_id, $final_amount = null , $import_batch = null)
    {
        
        $status = $this->calculatePaymentStatusupdateImport($transaction_id, $final_amount , $import_batch);
        Transaction::where('id', $transaction_id)
            ->update(['payment_status' => $status]);
        ...
        foreach ($transaction_lines as $line)
         {
            ...
            // dd($line->imei_number);
            if(isset($line->imei_number)){
            PurchaseLine::where('imei_number' , $line->imei_number)->update(['imei_number_status' => 1]);
            }  
        }
    }
  ...
  public function payContacts($request, $format_data = true)
    {
    ...
    public function getPdfContentsForGivenTransaction($business_id, $transaction_id)
    {
        ...
        //Get receipt details
        $receipt_details = $this->getReceiptDetails($transaction_id, $transaction->location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type , $business_id);
        $currency_details = [

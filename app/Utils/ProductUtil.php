...
class ProductUtil extends Util
{
    ...
    {        
        // dd($product,$intergirdent,$data,$mfg,$locationid);
        ...
        foreach ($mfg as  $value) {
                        ...
                        $old_quan=DB::table('variation_location_details')->where('variation_id', $value->variation_id)
...
// saad
  ...
  public function updateproductionquantity($previous_quanity, $product, $intergirdent, $data, $mfg, $locationid)
    {


        $alll = DB::table('mfg_recipes')->where('variation_id', $product['variation_id'])->first();
        if ($product['quantity'] != $previous_quanity) {
            $qan =  $product['quantity'] - $previous_quanity;
        } else {
            $qan = 0;
        }
        if ($qan != 0) {
    ...
    public function decreaseProductQuantityCombo($combo_details, $location_id,$mfg,$intergirdent,$locationid,$varid,$qut)
    {
        ...
        // $qan=$qut;
        // $dd= json_encode($combo_details);
        //     print_r($dd);

        foreach($combo_details as $cb){
    ...
    public function decreaseProductQuantityComboImport($combo_details, $location_id)
    {
        //product_id = child product id
        //variation id is child product variation id
        foreach ($combo_details as $details) {
     ...
     */
     ...
     public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true, $imei_number = null ,$selectedValue = null)
     {
        
         $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
                 ->join('product_variations AS pv', 'variations.product_variation_id', '=', 'pv.id')
                 ->leftJoin('purchase_lines', function ($join) {
                    $join->on('p.id', '=', 'purchase_lines.product_id')
                         ->where('purchase_lines.imei_number_status', '=', 0);
                })
         ...
         if ($selectedValue == "aed") {
             $product = $query->select(
                 ...
                 'p.barcode_type',
                 'vld.qty_available',
                 'variations.default_single_aud_dsp',
                ...
                DB::raw("(SELECT imei_number FROM purchase_lines 
          WHERE '$imei_number' = purchase_lines.imei_number 
          AND imei_number_status = 0 
          ORDER BY id DESC 
          LIMIT 1) as imei_number")

             )
             ->firstOrFail();
         }elseif ($selectedValue == "usd") {
             $product = $query->select(
                 DB::raw("IF(pv.is_dummy = 0, CONCAT(p.name, 
                         ' (', pv.name, ':',variations.name, ')'), p.name) AS product_name"),
                 'variations.default_purchase_price',
                 ...
                 'p.barcode_type',
                 'vld.qty_available',
                 'variations.default_single_dollar_dsp',
                 'variations.sell_price_inc_tax',
                 'variations.id as variation_id',
                 'variations.combo_variations',  //Used in combo products
                 'units.short_name as unit',
          ...
          LIMIT 1) as imei_number")
             ...
             $product = $query->select(
                 ...
                 'units.allow_decimal as unit_allow_decimal',
                 'brands.name as brand',
                //   'purchase_lines.imei_number_status',
             
                 DB::raw("(SELECT purchase_price_inc_tax FROM purchase_lines WHERE 
                 variation_id=variations.id ORDER BY id DESC LIMIT 1) as last_purchased_price"),

                 DB::raw("(SELECT imei_number_status FROM purchase_lines WHERE 
                 '$imei_number' = purchase_lines.imei_number AND 
                 (purchase_lines.imei_number_status = 1 OR purchase_lines.imei_number_status = 0) 
                 ORDER BY id DESC LIMIT 1) as imei_number_status"),
        ...
        //  PurcahseLine::where()
         ...
         if($product->imei_number == NULL && $product->imei_number_status == 1){
            return "";
         }else{
            return $product;
         }
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $key => $data) 
        {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                if (isset($data['prefix_number']) && $data['postfix_number']) {

                $prefix_number[] = $data['prefix_number'];

                $postfix_number[] = $data['postfix_number'];

                $imei_number[] = $data['prefix_number'].$data['postfix_number'];
                ...
                $purchase_line->imei_number = $imei_number[$key];
                $purchase_line->prefix_number = $prefix_number[$key];
                $purchase_line->postfix_number = $postfix_number[$key];
            ...
            elseif(!empty($data['prefix_number']) && !empty($data['postfix_number']) && $data['quantity'] >= 1)
            {
                    // dd($data);
                // dd($new_quantity_f);
                if ($transaction->status == 'received') {
                    ...
                    $imei_prefix = $data['prefix_number'];
                    $imei_postfix_start = $data['postfix_number'];
                    $imei_quantity = $data['quantity'];

                    $imei_numbers = []; // Initialize an array to store IMEI numbers.
                    $imei_postfixes = []; // Initialize an array to store Psotfix numbers.

                 
                    for ($i = 0; $i < $imei_quantity; $i++) {

                        $imei_postfix = sprintf("%03d", $imei_postfix_start + $i);
                        $imei_number = $imei_prefix . $imei_postfix;
                        $imeo_postfix = $imei_postfix;
                        // Store each IMEI number in the array.
                        $imei_numbers[] = $imei_number;
                        $imei_postfixes[] = $imeo_postfix;
                        ...
                        for ($i = 0; $i < $new_quantity; $i++) {
                            ...
                            $purchase_line->imei_number = $imei_numbers[$i];
                            $purchase_line->prefix_number = $imei_prefix;
                            $purchase_line->postfix_number = $imei_postfixes[$i];
                            $purchase_line->quantity = 1; // IMEI quantity is always 1

                            
                            // dd("d");
                            $purchase_line->pp_without_discount = ($this->num_uf($data['pp_without_discount'], $currency_details) * $exchange_rate) / $multiplier;
                            // dd($purchase_line->pp_without_discount);
                            $purchase_line->discount_percent = $this->num_uf($data['discount_percent'], $currency_details);
                            $purchase_line->purchase_price = ($this->num_uf($data['purchase_price'], $currency_details) * $exchange_rate) / $multiplier;
                            $purchase_line->purchase_price_inc_tax = ($this->num_uf($data['purchase_price_inc_tax'], $currency_details) * $exchange_rate) / $multiplier;
                            $purchase_line->item_tax = ($this->num_uf($data['item_tax'], $currency_details) * $exchange_rate) / $multiplier;
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        if (in_array('imei_number', $search_fields)) {
            $query->leftjoin('purchase_lines as pls', 'products.id', '=', 'pls.product_id');
        }
        ...
        if (! empty($search_term)) {
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('imei_number', $search_fields)) {
                        ...
                        $query->orWhere('pls.imei_number', 'like', '%'.$search_term.'%');
                    }
            ...
            if ($search_type == 'exact') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('imei_number', $search_fields)) {
                        $query->orWhere('pls.imei_number', $search_term);
                    }
                });
            }
        }
        ...
        if (in_array('imei_number', $search_fields)) {
            $query->addSelect('pls.id as purchase_line_id', 'pls.imei_number');
        }
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        $products = $query->select(
            ...
            DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * purchase_price_inc_tax) FROM transactions 
                ...
                //   imei_number
                // DB::raw('(SELECT GROUP_CONCAT(pl.imei_number SEPARATOR "|") 
                //         FROM transactions
                //         JOIN purchase_lines AS pl ON transactions.id = pl.transaction_id
                //         JOIN products ON products.id = pl.product_id
                //         WHERE pl.transaction_id = transactions.id) AS imei_number'),
                // imei_number_status
                        // DB::raw('(SELECT GROUP_CONCAT(pl.imei_number_status SEPARATOR "|") 
                        // FROM transactions
                        // JOIN purchase_lines AS pl ON transactions.id = pl.transaction_id
                        // JOIN products ON products.id = pl.product_id
                        // WHERE pl.transaction_id = transactions.id) AS imei_number_status'),

                        
                //         DB::raw('(SELECT GROUP_CONCAT(pl.imei_number SEPARATOR "|") 
                // FROM purchase_lines AS pl 
                // WHERE pl.transaction_id = t.id 
                // AND pl.variation_id = variations.id) AS imei_number'),

                 // imei_number_status
                //  DB::raw('(SELECT GROUP_CONCAT(pl.imei_number_status SEPARATOR "|") 
                //  FROM purchase_lines AS pl 
                //  WHERE pl.transaction_id = t.id 
                //  AND pl.variation_id = variations.id) AS imei_number_status'),
            ...
            DB::raw("(SELECT GROUP_CONCAT(transaction_sell_lines.imei_number SEPARATOR '|') 
            FROM transactions 
            JOIN transaction_sell_lines AS transaction_sell_lines ON transactions.id=transaction_sell_lines.transaction_id
            WHERE transactions.status='final' 
            AND transactions.type='sell' 
            AND transactions.location_id=vld.location_id
            AND transaction_sell_lines.variation_id=variations.id
            LIMIT 1) as imei_number"),


//                 DB::select(DB::raw("
//     SELECT GROUP_CONCAT(transaction_sell_lines.imei_number SEPARATOR '|') as imei_numbers
//     FROM transactions 
//     JOIN transaction_sell_lines AS transaction_sell_lines ON transactions.id=transaction_sell_lines.transaction_id
//     WHERE transactions.status='final' 
//     AND transactions.type='sell' 
...
// ")),
        ...
        //           DB::raw("(SELECT TSL.imei_number 
        //  FROM transactions 
        //  JOIN transaction_sell_lines AS TSL ON transactions.id = TSL.transaction_id
        //  JOIN variations ON TSL.variation_id = variations.id
        //  WHERE transactions.status = 'final' 
        //  AND transactions.type = 'sell' 
        //  AND transactions.location_id = vld.location_id
        //  AND variations.product_id = (SELECT id FROM products WHERE name = 'ABC')
        //  ) as imei_number"),


            DB::raw('SUM(vld.qty_available) as stock'),
           // for only view list products 
            'variations.sub_sku as sku',
            'p.name as product',
            // in below onhly for stock product report
            'variations.sub_sku',
            'p.name',
            'p.type',
            'p.alert_quantity',
    ...
    public function createOrUpdatePurchaseLinesImport($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null){
        ...
        foreach ($input_data as $key => $data) 
        {
            ...
            $purchase_line->purchase_order_line_id = ! empty($data['purchase_order_line_id']) ? $data['purchase_order_line_id'] : null;
            $purchase_line->purchase_requisition_line_id = ! empty($data['purchase_requisition_line_id']) && $transaction->type == 'purchase_order' ? $data['purchase_requisition_line_id'] : null;
            $purchase_line->quantity_sold = $quantity_sold;
    ...
    public function setAndGetReference($type)
    {
        $user = Auth::user();
        $business_id = $user->business_id;

            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\CurrencyChangeController::class, 'index']),
                                'CurrencyChange',
                                ['icon' => 'fa fas fa-gem', 'active' => request()->segment(1) == 'CurrencyChange']
                            );
                        }
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class , 'oder_reports']),
                            __(' Service Staff Order Reports'),
                            ['icon' => 'fa fas fa-arrow-circle-down', 'active' => request()->segment(2) == 'oder_reports-report']
                        );
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'getkot']),
                            __('Kot Report'),
                            ['icon' => 'fa fas fa-user-secret', 'active' => request()->segment(2) == 'get-kot']
                        );

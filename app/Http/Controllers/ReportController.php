...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $datatable = Datatables::of($products)
                ...
                ->addColumn('sale_imei_number', function ($row) 
                {
                    if (!empty($row->imei_number)) {
                        $imei_numbers = $row->imei_number;
                        $imeiArray = explode('|', $imei_numbers);
                        $imeiDisplay = '';
                        
                        foreach ($imeiArray as $index => $imei) {
                ...
                $raw_columns = ['unit_price', 'total_transfered', 'total_sold',
                    ...
                    'potential_profit', 'action' , 'sale_imei_number'];
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->join('contacts as c', 't.contact_id', '=', 'c.id')
                ->join('products as p', 'pv.product_id', '=', 'p.id')
                ->join('users', 't.created_by', '=', 'users.id')
        ...
        return view('report.product_sell_report')
            ->with(compact('business_locations', 'customers', 'categories', 'brands',
                'customer_group', 'product_custom_field1','users', 'product_custom_field2'));
    }
    ...
    public function getProfit($by = null)
    {
        ...
        //         (
        //             SELECT IFNULL(
        //                 SUM((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.purchase_price_inc_tax)),
        //                 0
        //             ) AS total
        //             FROM transaction_sell_lines AS tsl
    ...
    public function oder_reports(Request $request,$sale_type = 'sell')
    {
        //  $business_id = $request->session()->get('user.business_id');
        ...
        $query = TransactionSellLine::join(
            ...
            ->leftJoin('users as ss', 't.res_waiter_id', '=', 'ss.id')
            ...
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.res_waiter_id','!=' ,'null')
            ->where('t.payment_status','=' ,'due')
            ->select(
                'p.name as product_name',
                'bl.name as location_name',
                'p.type as product_type',
                ...
                'transaction_sell_lines.parent_sell_line_id',
                //   DB::raw('((SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) * transaction_sell_lines.unit_price_inc_tax) as subtotal'),
                DB::raw('ROUND(((SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) * transaction_sell_lines.unit_price_inc_tax), 2) as subtotal'),
           ...
           if (request()->has('location_id')) {
            ...
            if (!empty($request->input('start_date'))) {
                ...
                $query ->whereBetween(DB::raw('DATE(transaction_sell_lines.created_at)'), [$newDate, $newDate1]);
            }
            ...
            return Datatables::of($query)
                ...
                ->editColumn(
                    'sell_qty',
                    '<span class="sell_qty" data-orig-value="{{$sell_qty}}">{{$sell_qty}}</span>'
                )
                  ->editColumn(
                    'subtotal',
                    '<span class="subtotal" data-orig-value="{{$subtotal}}">{{$subtotal}}</span>'
                )
                
                ->addColumn('checkbox', function ($row) {
                })
                ->rawColumns(['sell_qty','subtotal'])
                ->make(true);
        }   
        $users = User::where('business_id', $business_id)->get();
         $category = Category::where('business_id', $business_id)->get();
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        return view('report.oder_reports')->with(compact('users','business_locations','category'));
    }
    ...
    public function getkot(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $end_date = $request->get('end_date');
            if (!empty($start_date) && !empty($end_date)) {
                $kot->whereDate('transactions.transaction_date', '>=', $start_date)
                    ->whereDate('transactions.transaction_date', '<=', $end_date);
            }
            return Datatables::of($kot)

                ->rawColumns([])
                ->make(true);
        }

        return view('report.kot');
    }


    // Zaheer work for Stock Repotd
    public function EnterStock(Request $request)
    {
        ...
        $updateData = [
            ...
            'enter_current_stock' => $request->input('enter_current_stock'),

        ];

        $result = VariationLocationDetails::where('product_id', $product_id)->where("variation_id" , $variation_id)->update($updateData);


        if ($result) {
            return response('Data saved successfully!', 200);
        } else {
            return response('Data save failed.', 500);
        }
    }

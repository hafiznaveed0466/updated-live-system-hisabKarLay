...
class HomeController extends Controller
{
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            // $getPurchaseDuePaid =  $this->transactionUtil->getPaidDue($business_id, $start, $end, $location_id, 'purchase');
          
            // $getSellDuePaid     =  $this->transactionUtil->getPaidDue($business_id, $start, $end, $location_id, 'sell');
           
            $query   =  $this->transactionUtil->getpaid($business_id, $start, $end, $location_id);
// dd($query);
            $transaction_types = [
                'purchase_return', 'sell_return', 'expense','paid'
            ];
            ...
            $output['cashReceived']     =  $query;
            $output['cashhand'] = $query - $purchase_details['totals_due']-$total_sell_return_inc_tax-$transaction_totals['total_expense'] ;
            $output['total_due'] = $purchase_details['totals_due'];

...
class SellController extends ApiController
{
    ...
    public function index()
    {
        ...
        $with = ['sell_lines.modifiers', 'payment_lines', 'contact','res_tables','type_of_services','sell_lines.variations', 'sell_lines.variations.product_variation'];
        $query = Transaction::where('transactions.business_id', $business_id)->select('transactions.*',DB::raw("DATE_FORMAT(transactions.transaction_date, '%Y-%m-%d %h:%i:%s %p') as transaction_date"))->where('type', 'sell');
...
// dd($sells);
    ...
    public function show($sell_ids)
    {
        ...
        $with = ['sell_lines.product','sell_lines.modifiers','payment_lines','res_tables','type_of_services','sell_lines.variations', 'sell_lines.variations.product_variation', 'contact' , 'service_staff'];
...
//                      $ac =$this->transactionUtil->createOrUpdateSellLines($transaction, $input['products'], $input['location_id'], false, null, [], false);
...
//                                 // $data = $this->productUtil->productionquantity($product ,$data,$mfg,
...
//                                                             //  print_r(json_encode($intergirdent));
//                                                             // $datas= $this->productUtil->decreaseProductQuantityCombo(
//                                                             // $product['combo'],
//                                                             // $input['location_id'],
...
//                                                         $old_quan=DB::table('variation_location_details')->where('variation_id', $varid)->where('location_id',$input['location_id'])
//                                                         ->get();
                        
                          
                        
//                                                     $old_respie_material=json_decode($old_quan);
//                                                     foreach ($old_respie_material as $update) {
//                                                         $vid=$update-> variation_id ;
//                                                         foreach ($product['combo'] as $value) {
//                                                             // dd($value);
//                                                                 if ($vid==$value["variation_id"]) {
//                                                                         $productquan=$value["quantity"];
//                                                                         $oldquan=$update->qty_available;
//                                                                     $remaingqut=$oldquan-$productquan;
//                                                                     VariationLocationDetails::where('variation_id', $varid)->where('location_id',$input['location_id'])
//                                                                     ->update([
//                                                                         'qty_available' => $remaingqut
//                                                                         ]);
 ...
 public function store(Request $request)
    {
            ...
            foreach ($sells as $sell_data) {
                ...
                // try {
                    ...
                    $transaction->payment_link = $this->transactionUtil->getInvoicePaymentLink($transaction->id, $business_id);
                                        ...
                                        // only get tables and servies

                    if (isset($sell_data['table_id'])) {
                         $table_id = $sell_data['table_id']; 
                         $table = ResTable::find($table_id);
                         $transaction['res_tables'] = $table;
                    }
                    if (isset($sell_data['types_of_service_id'])) {
                        
                    $service_staff_id = $sell_data['types_of_service_id'];
                    $service = TypesOfService::find($service_staff_id);
                    
                    $transaction['type_of_services'] = $service;
                    }
                    
                    DB::commit();
              foreach ($transaction->sell_lines as &$sell_line) {
            if (isset($sell_line['variation_id'])) {
                $variation = Variation::find($sell_line['variation_id']);
                ...
                $sell_line['product'] = [
                    ...
                    'sku' => $product->sku,
                    'type' => $product->type,
                     // ... (other variation properties you want to include)
                    'variation' => [
                        'name' => $variation->name,
                        // ... (other variation properties you want to include)
                    ]
        ...
        // Create a new entry in the response data array
        // $responseItem = [
            // 'transaction' => $transaction,
            // 'products' => $input['products'],
        // ];

        // Add the response item to the output array
        $output[] = $transaction;
    ...
    */
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $updated_transaction = Transaction::where('business_id', $user->business_id)->with(['sell_lines' ,'payment_lines' , 'res_tables' , 'type_of_services'])
                                    ->findOrFail($id);
            ...
            $updated_transaction->payment_link = $this->transactionUtil->getInvoicePaymentLink($updated_transaction->id, $business_id);
             ...
             foreach ($updated_transaction->sell_lines as &$sell_line) {
                if (isset($sell_line['variation_id'])) {
                    $variation = Variation::find($sell_line['variation_id']);
                    $product = Product::find($sell_line['product_id']);
  ...
  private function __formatSellData($data, $transaction = null)
    {
        ...
        if (!empty($data['products'])) {
            ...
            foreach ($data['products'] as $product_data) {
                ...
                // Retrieve modifiers data correctly using the right variable names
                $modifiers = $this->__getValue('modifier', $product_data, $sell_line);
                $modifier_prices = $this->__getValue('modifier_price', $product_data, $sell_line);
                $modifier_quantities = $this->__getValue('modifier_quantity', $product_data, $sell_line);
                $modifier_set_ids = $this->__getValue('modifier_set_id', $product_data, $sell_line);
                ...
                if (!empty($modifiers)) {
                    foreach ($modifiers as $index => $modifier) {
                        $modifiersData[] = [
                            ...
                            'modifier_price' => $modifier_prices[$index] ?? 0,
                            'modifier_quantity' => $modifier_quantities[$index] ?? 1,
                            'modifier_set_id' => $modifier_set_ids[$index] ?? null,
                        ];

                       
                        $modifiers_total += $modifier_prices[$index] * $modifier_quantities[$index];
                ...
                $product_id = $this->__getValue('product_id', $product_data, $sell_line);
                $imei_number = $this->__getValue('imei_number', $product_data, $sell_line);
                $variation_id = $this->__getValue('variation_id', $product_data, $sell_line);
                $product = Product::where('business_id', $business_id)
                                ->with(['variations'])
                                ->findOrFail($product_id);
                ...
                // $line_total = $unit_price_inc_tax * $formated_sell_line['quantity'];
                $line_total = $modifiers_total + ($unit_price_inc_tax * $formated_sell_line['quantity']);
    ...
    $packing_charge = $this->__getValue('packing_charge', $data, $transaction, 0);
        ...
        if (!empty($packing_charge) && !empty($formated_data['types_of_service_id']) &&  !empty($packing_charge_type == 'percent')) 
           {
           if ($order_discount_type == 'percentage') 
            {

            $packing_charge_percentage = ($packing_charge / 100) * $subtotal;
            $final_total += $packing_charge_percentage;
            }elseif($order_discount_type == 'fixed')
            {
                $packing_charge_percentage = ($packing_charge / 100) * $subtotal ;
                $final_total += $packing_charge_percentage;
            ...
            $packing_charge_percentage = ($packing_charge / 100) * $final_total;
            $final_total += $packing_charge_percentage;
        ...
        if (!empty($order_tax_id)) {
            ...
            $final_total += $order_tax;
             ...
             if (!empty($packing_charge) && !empty($formated_data['types_of_service_id']) && !empty($packing_charge_type == 'fixed')) {
        ...
        $formated_data['tax_calculation_amount'] = $order_tax;
         ...
         $formated_data['packing_charge'] = $packing_charge;
        $formated_data['packing_charge_type'] = $packing_charge_type;

        $final_total += $formated_data['shipping_charges'];
        
      
        // $formated_data['final_total'] = $final_total  + $modifiers_total;
         $formated_data['final_total'] = $final_total;
        ...
        if (!empty($data['payments'])) {
            foreach ($data['payments'] as $payment_data) {
                ...
                $modifiers_total_payment = 0;
        foreach ($formated_data['products'] as $product) {
            foreach ($product['modifier'] as $modifier) {
                // Ensure the modifier_price and modifier_quantity exist
                if (isset($modifier['modifier_price']) && isset($modifier['modifier_quantity'])) {
                    $modifiers_total_payment += ($modifier['modifier_price'] * $modifier['modifier_quantity']);
                }
            }
        }

        // Add the modifiers total amount to the payment
               $payment['modifiers_total'] = $modifiers_total_payment;
  ...
  public function kot($id)
        ...
        $data = DB::table('transaction_sell_lines')
            ...
            ->join('business_locations', 'business_locations.id', '=', 'transactions.business_id')
             ...
             ->select(
                ...
                'transaction_sell_lines.new_qty',
                ...
                DB::raw("DATE_FORMAT(transactions.transaction_date, '%Y-%m-%d %h:%i:%s %p') as transaction_date"),
                'contacts.name',
    ...
    public function bill($id)
    {
        ...
        $data = DB::table('transaction_sell_lines')
            ...
            ->select(
            'transactions.id', 'transactions.invoice_no',DB::raw("DATE_FORMAT(transactions.transaction_date, '%Y-%m-%d %h:%i:%s %p') as transaction_date"),'transaction_sell_lines.quantity', 
             'products.name', 'contacts.name as cname', 'business_locations.mobile',
            'business_locations.country', 'business_locations.city', 'business_locations.state', 'business_locations.zip_code',
            'business_locations.name as bname', 'business_locations.landmark', 'transaction_sell_lines.created_at', 'transactions.final_total',
            'transaction_sell_lines.unit_price')->where('transactions.id', '=', $id)->where('transaction_sell_lines.parent_sell_line_id', '=', $ab)

...
<section class="content no-print">
    ...
    @endcomponent
           {{-- Import Purchase --}}
           {!! Form::open(['url' => action([\App\Http\Controllers\PurchaseController::class, 'ImportPurchase']), 'method' => 'post', 'enctype' => 'multipart/form-data' ]) !!}
           <div class="row">
               <div class="col-sm-6">
               <div class="col-sm-8">
                   <div class="form-group">
                       {!! Form::label('name', __( 'Purchase Import' ) . ':') !!}
                       {!! Form::file('purcahse_csv', ['accept'=> '.xls, .xlsx, .csv', 'required' => 'required']); !!}
                     </div>
               </div>
               <div class="col-sm-4">
               <br>
    ...
    @component('components.widget', ['class' => 'box-primary', 'title' => __('purchase.all_purchases')])
        @can('purchase.create')
            @slot('tool')
             ...
             {{-- purchase export --}}
             <a href="{{ action('\App\Http\Controllers\PurchaseController@exportPurchase') }}" class="btn btn-success" style="margin-left: 10px;">Export Purchases</a>
             {{-- purchase export --}}

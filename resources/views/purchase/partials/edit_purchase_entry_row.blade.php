...
<div class="table-responsive">
    ...
    id="purchase_entry_table">
        <thead>
              <tr>
                ...
                {{-- New by zaheer --}}
                @if(isset($common_settings['enable_purchase_imei']))
                              
								<th>{{__('Prefix Number')}}</th>
								<th>{{__('Postfix Number')}}</th>
    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        <tr @if(!empty($purchase_line->purchase_order_line) && !empty($common_settings['enable_purchase_order'])) data-purchase_order_id="{{$purchase_line->purchase_order_line->transaction_id}}" @endif  @if(!empty($purchase_line->purchase_requisition_line) && !empty($common_settings['enable_purchase_requisition'])) data-purchase_requisition_id="{{$purchase_line->purchase_requisition_line->transaction_id}}" @endif>
            ...
            <td>
                {!! Form::text('purchases[' . $loop->index . '][prefix_number]', $purchase_line->prefix_number, ['class' => 'form-control input-sm  input_number' ,  'required']); !!}
            </td>

            <td>
                {!! Form::text('purchases[' . $loop->index . '][postfix_number]', number_format($purchase_line->postfix_number), ['class' => 'form-control input-sm  input_number' ,  'required']); !!}
            </td>
            @endif

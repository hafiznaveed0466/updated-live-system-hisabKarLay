...
<table class="table table-bordered table-striped" id="stock_report_table">
    <thead>
        <tr>
            ...
            {{-- @dd(session()->get('business.common_settings.enable_product_enter_stock_diff')); --}}
            {{-- Zaheer --}}
            @if(!empty(session()->get('business.common_settings.enable_product_enter_stock_diff')))
            <th class="action_current_stock_enter">Action</th>
            @endif  
            <th>SKU</th>
            <th>@lang('business.product')</th>
            
            <input type="hidden" class="sale_out_imei_number" value="{{ auth()->user()->business_id }}">
            {{-- @if (auth()->user()->business_id == 29) --}}
            <th class=>{{ __('Sale Out IMEI Number') }}</th>
            {{-- @endif --}}
            ...
            @endif
             {{-- Zaheer --}}
             @if(!empty(session()->get('business.common_settings.enable_product_enter_stock_diff')))
             <th class="current_stock_enter">{{__('Enter Current Stock') }} </th>
             @endif
